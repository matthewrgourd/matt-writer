...
async function fetchAllPostUrls() {
  ...
  while (true) {
    const apiUrl = `https://${SUBSTACK_SUBDOMAIN}.substack.com/api/v1/posts?limit=${limit}&offset=${offset}`;
    const response = await fetch(apiUrl, {
      headers: { 'User-Agent': 'matt-writer-substack-sync/1.0' },
    });
    ...
    for (const post of posts) {
      urls.push(`https://substack.com/home/post/p-${post.id}`);
    }
...
function sortByTitleDesc(posts) {
...
async function main() {
  ...
  const urls = syncAll ? await fetchAllPostUrls() : SOURCE_POSTS;
  ...
  const sorted = syncAll ? sortByTitleDesc(posts) : sortByPublishedDesc(posts);
